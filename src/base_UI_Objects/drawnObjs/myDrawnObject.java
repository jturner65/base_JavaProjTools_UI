...
public abstract class myDrawnObject {
	...
	public myDrawnObject(my_procApplet _pa, myVector _canvNorm) {
		pa = _pa;
...
class myVariStroke extends myDrawnObject {
	...
	public void handleMouseDrag(myVector dispVec, int drawnTrajPickedIdx){		
		...
		int minBnd = MyMathUtils.max(drawnTrajPickedIdx - pa.drawnTrajEditWidth, 0),
			maxBnd = MyMathUtils.min(drawnTrajPickedIdx + pa.drawnTrajEditWidth, pts.length-1);		
		...
		for(int idx = minBnd; idx < maxBnd; ++idx){
			...
			modAmt = (float) (pa.trajDragScaleAmt* Math.cos((idx-drawnTrajPickedIdx) * MyMathUtils.halfPi * divMultVal));//trajDragScaleAmt/abs(1 + (idx-drawnTrajPickedIdx));
			//modAmt *= modAmt;
...
abstract class myOffset {
	...
	public myOffset(my_procApplet _pa, boolean _ec){
		pa = _pa;
		ID = IDcnt++;
		endCaps = _ec;
	}	
	
	public myOffset(my_procApplet _pa){this(_pa, true);}
